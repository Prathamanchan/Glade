...
if geomNumShapes(nwell) > 0 :
	...
	geomSpace(nwell, 1.2, samenet, "NW space < 0.62 (NW.S.1)")
	geomSpace(nwell, 1.5, diffnet, "NW space (different net) < 1.0 (NW.S.2)")
...
if geomNumShapes(metal1) > 0 :
	...
	##geomSpace2(metal1, 0.6, 10.0, 1.0, 0, "M1 space with M1 width > 10 < 0.6 (M1.S.2)")
	# add parallel rule here
	geomEnclose2(metal1, cont, 0.0, 0.05, 0.05, 2, "M1 enclosure of CO (M1.EN.2)")
...
if geomNumShapes(via12) > 0 :
	...
	geomEnclose2(metal1, via12, 0.01, 0.01, 0.05, 2, "M1 enclosure of VIA1 (VIA1.EN.2)")

if geomNumShapes(metal2) > 0 :
	print "Check metal2"
	geomWidth(metal2, 0.16, "M2 width < 0.16 (M2.W.1)")
	geomSpace(metal2, 0.18, 0, "M2 space < 0.18 (M2.S.1)")
	##geomSpace2(metal2, 0.6, 10.0, 0, "M2 space with M2 width > 10 < 0.6 (M2.S.2)")
	# add parallel rule here
	geomEnclose2(metal1, via12, 0.005, 0.05, 0.05, 2, "M2 enclosure of VIA1 (M2.EN.2)")
	geomArea(metal2, 0.122, 9e99, "M2 area < 0.122 (M2.A.1)")
...
if geomNumShapes(metal3) > 0 :
	print "Check metal3"
	geomWidth(metal3, 0.16, "M3 width < 0.16 (M3.W.1)")
	geomSpace(metal3, 0.18, 0, "M3 space < 0.18 (M3.S.1)")
	##geomSpace2(metal3, 0.6, 10.0, 0, "M3 space with M3 width > 10 < 0.6 (M3.S.2)")
	# add parallel rule here
	geomEnclose2(metal3, via23, 0.005, 0.05, 0.05, 2, "M3 enclosure of VIA2 (M3.EN.2)")
	geomArea(metal3, 0.122, 9e99, "M3 area < 0.122 (M3.A.1)")
