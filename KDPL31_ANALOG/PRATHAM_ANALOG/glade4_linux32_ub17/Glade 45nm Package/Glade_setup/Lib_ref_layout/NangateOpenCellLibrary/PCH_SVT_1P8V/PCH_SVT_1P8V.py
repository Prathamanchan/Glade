#------------------------------------------------------------------------------
#
# NMOS Pcell for PCH_SVT_1.8V device. 
...
def PCH_SVT_1P8V(cv, w=0.42, l=0.18, m=1, polyCnt=0, leftCnt=1, rightCnt=1) :
	lib = cv.lib()
	dbu = lib.dbuPerUU()
	tech = lib.tech()
	width = int(w * dbu)
	...
	sdnwell_ovlp_pdiff = int(0.12 * dbu)
	poly_to_cont = int(0.16 * dbu)
	active_ovlp_cont = int(0.10 * dbu)

	diff_to_diff_space = int(0.28 * dbu)
	ndiff_ovlp_nwell = int(0.43 * dbu)
	poly_extn_diff = int(0.22 * dbu)
	pdiff_ovlp_nwell = int(0.12 * dbu)
	pdiff_ovlp_pimp = int(0.18 * dbu)
	...
	r = Rect(-height/2 - ndiff_ovlp_nwell,
		-width/2 - ndiff_ovlp_nwell,
		height/2 + ndiff_ovlp_nwell,
		+width/2 + ndiff_ovlp_nwell,)
	...
	r = Rect(-height/2 - pdiff_ovlp_pimp, 
		 -width/2 - pdiff_ovlp_pimp, 
		 height/2 + pdiff_ovlp_pimp,
		 width/2 + pdiff_ovlp_pimp)
