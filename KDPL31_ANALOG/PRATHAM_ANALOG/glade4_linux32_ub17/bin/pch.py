...
def pch(cv, w=1.1e-6, l=0.13e-6, m=1, polyCnt=0, leftCnt=1, rightCnt=1) :
	lib = cv.lib()
	...
	r = Rect(-height/2 - pplus_ovlp_active, 
		 -width/2 - pplus_ovlp_active, 
		 height/2 + pplus_ovlp_active,
		 width/2 + pplus_ovlp_active)
	...
	for i in range(fingers) :
		...
		if polyCnt :
			...
			if length < poly_via_size :
				...
				cv.dbCreatePort(pin, metpin)
			if length < poly_via_size :
				xfudge = poly_ovlp_cut - poly_via_neck_offset
			else :
				xfudge = (length - (numCuts*cut_width + (numCuts-1)*cut_space) ) / 2
			for j in range(numCuts) :
				r = Rect(xoffset + xfudge + (j * (cut_width + cut_space)),
				 	width/2 + poly_ovlp_active + poly_ovlp_cut,
				 	xoffset + xfudge + (j * (cut_width + cut_space)) + cut_width,
